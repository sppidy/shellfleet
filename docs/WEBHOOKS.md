...
Each delivery is independent: one sink failing doesn't block another.
Every attempt audits a row at `/activity` with shape
`webhook.<event_kind> · sink={generic|slack|discord|telegram} · code=<status>`,
so a Discord 503 mid-rollout is visible immediately.
...
Body shape is the same across events: pre-rendered headline + status +
optional log tail + optional error string + agent_id.

## Configuring sinks

### One config for everything (good starting point)

Set the prefix-less env vars on the server. Every event lands in the
same sink:

```bash
# Set whichever sinks you want. Configured ones fire; unset ones don't.
...
prefix-less version. Use this when one event should route to a louder
channel:
...
Example: keep the daily noise quiet but escalate disconnects:
...
`update_result`, `health_probe.transition`, `backup_job.result` land at
the Slack URL; `agent.disconnect` lands at the Telegram bot *instead of*
the Slack URL (per-event override wins, not adds).

### Disconnect / reconnect debounce

Agent restarts (systemd / kubelet rolling out a new build, brief network
blips, Cloudflare idle drops) all manifest as a WS read-loop exit on the
server. To avoid spamming your Telegram, the disconnect webhook is gated
by a grace window:
...
- **Quick reconnect (under the grace)**: silent. No webhook fires.
- **Confirmed offline (past the grace)**: `agent.disconnect` fires once.
- **Coming back after a confirmed disconnect**: `agent.connect` fires once.
- **First-time agent connect**: silent. Connect events only fire when
  there's an outstanding "this agent is gone" notice to clear.
...
All chat formats truncate the log to stay under each provider's limit
(Discord caps at 2000 chars, Telegram at 4096). The structured-JSON sink
doesn't truncate.
...
Every fire (success or failure) writes one row per sink at `/activity`:
...
- **Auth + admin events.** Sign-in, MFA enable/disable, role change,
  token revoke. The audit log at `/activity` covers them. Webhook for
